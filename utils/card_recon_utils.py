...
def split_debit_credit(records):
    """分离借方（收款）和贷方（退款）"""
    charges = [r for r in records if r["type"] == "借方"]
    refunds = [r for r in records if r["type"] == "贷方"]
    return charges, refunds


def _match_by_amount(pms_txs, bank_txs, tolerance=0.01):
    """同金额分组配对算法。先精确匹配，同金额多笔时按时间就近匹配。
...
def _reconcile_channel(channel, pms_txs, bank_txs, ar_txs=None):
    """单个通道的对账逻辑"""
    # Step 2: 金额聚合对比
    pms_total = sum(t["amount"] for t in pms_txs)
    bank_total = sum(t["amount"] for t in bank_txs)
    bank_fees = sum(t["fee"] for t in bank_txs)
