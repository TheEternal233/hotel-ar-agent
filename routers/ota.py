...
def _ota_confirm_sync(req: OtaConfirmRequest):
    try:
        ...
        audit.log("ota_recon", "confirm", f"OTA对账确认完成: {channel}, 匹配{stats.get('match',0)}笔差异{stats.get('diff',0)}笔",
                  context={"channel": channel, "stats": stats, "report_path": report_path})
